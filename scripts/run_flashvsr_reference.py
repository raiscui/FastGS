...
这条脚本已经从 `lyra` 收编到 FastGS.
它的定位是:
1. 先把 `view_id/rgb/*.mp4` 做成可复用的 SR reference.
2. 顺手导出逐帧对照图, 方便肉眼判断问题是不是从 SR 阶段开始出现.
3. 输出结果继续给后续 FastGS / 对比流程复用.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Run FlashVSR-Pro on view_id/rgb videos and export per-frame debug artifacts.",
    )
    parser.add_argument(
        "--input-root",
        type=str,
