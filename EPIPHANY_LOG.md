...
## [2026-03-28 02:24:00 UTC] [Session ID: 79642ac4-ccdf-404a-967b-1342f85cc2bd] 主题: 只看相机中心轨迹, 很容易把“pose 异常”和“材质难点”混在一起

### 发现来源
- 在 `my5_nomask_v1` 的 `worst-view -> pose` 深挖链路里得到.
- 这一轮是在已有:
  - 点支持
  - 相机中心轨迹
  的基础上, 又补了:
...
- 相机中心轨迹只能回答“相机去了哪里”.
- 它回答不了“相机在往哪里看”.
- 对高反射 / 弱纹理场景, 这两件事必须分开看.

### 为什么重要
- 如果少了“朝向连续性”这条证据, 很容易出现两种误判:
  - 把材质难点误判成 pose 错误
  - 或把真实 pose 抖动误判成“只是画面难”
- 这会直接把后续动作顺序带偏:
  - 是该继续训练
  - 还是先回到 COLMAP / matcher / 筛帧
...
- 更稳的 pose 诊断顺序应该是:
  ...
  4. 朝向连续性
  5. 邻机位相对位姿
- 当前 `my5` 的 `view 7` 之所以更像真实 pose 可疑点, 正是因为前 5 层证据已经开始互相对齐.
...
## [2026-03-28 02:49:00 UTC] [Session ID: 7f1d2edd-4a39-4aee-b0fd-79f9701c57e7] 主题: 缩小到局部 COLMAP 子集, 不一定会“把坏位姿修顺”, 反而可能进一步削弱点支持

### 发现来源
- 在 `my5` 的 `view 6 / 7 / 8 + frame 1 ~ 8` 局部 COLMAP 对照实验里得到.
- 这轮做了 2 版真实动态实验:
  - 固定内参
  - 允许 focal refine
...
- 如果没有这条经验, 后续很可能会继续:
  ...
  - 不断重跑小 COLMAP
  - 结果却只是让点支持更低, 平移还更差
- 这会把时间花在看起来“更精细”, 但其实证据收益很低的方向上.

### 未来风险
- 对高反射 / 弱纹理 / 几何约束弱的片段, 过度缩小 COLMAP 窗口可能带来:
  - 更低的 `observed` / `ratio`
  - 近似不变甚至更差的 `step`
  - 只有轻微变化的 `rotation`
- 如果只盯其中一个指标, 很容易误判成“局部已经更好”.

### 当前结论
- 当前 `my5` 的局部对照没有支持“全局上下文是主要问题”这个假设.
- 更稳的后续顺序应该改成:
  1. 先做筛帧对照
  2. 再考虑放大上下文窗口
  3. 最后才是继续局部 COLMAP 变体
...
## [2026-03-27 18:55:48 UTC] [Session ID: ebb3c562-1702-4ea2-8e98-ed18d0a9bada] 主题: `my5` 这种高反射疑难片段里, 单纯调 COLMAP 窗口大小不是高价值主旋钮

### 发现来源
- 在 `my5` 上连续完成了三档真实局部对照:
  - `view 6 / 7 / 8 + frame 1 ~ 8`
  - `view 6 / 7 / 8 + frame 1 ~ 8 + focal refine`
  - `view 5 / 6 / 7 / 8 + frame 1 ~ 12`

### 核心问题
- 直觉上很容易把局部 COLMAP 的下一步写成:
  - “窗口再缩一点”
  - 或“窗口再放大一点”
- 但这轮三档动态证据表明:
  - 两边都没有把 `view 7` 真正修顺
...
### 为什么重要
- 如果没把这条规律单独记下来, 后面很容易继续在“改窗口大小”上空转:
  - 小窗口让点支持下降
  - 大窗口让平移抖动和内参漂移变差
  - 但都没有换来决定性改善
...
### 当前结论
- 当前 `my5` 更稳的优先顺序应该改成:
  1. 先做带 frame-gap 保护的筛帧对照
  2. 再尝试更保守的 mapper / BA 参数
  3. 暂时不要继续围绕“缩窗口 / 放窗口”来回摆动

### 后续讨论入口
- 下次如果继续 `my5`, 先看:
  - `specs/my5_local_colmap_compare_20260328.md`
  - `specs/my5_local_pose_compare_v3_widecontext_assets/pose_continuity_data.json`
