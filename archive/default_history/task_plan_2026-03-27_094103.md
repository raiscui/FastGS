...
## [2026-03-26 17:50:56 UTC] [Session ID: 019d2b25-0539-7d51-8399-cffaae34ae9f] [支线索引]: `pixi run setup` 缺少 CUDA 开发头文件排障

- 已启用支线上下文集:
  - `task_plan__pixi_setup_cuda_headers.md`
  - `notes__pixi_setup_cuda_headers.md`
  - `WORKLOG__pixi_setup_cuda_headers.md`
  - `ERRORFIX__pixi_setup_cuda_headers.md`
- 启用原因:
  - 当前请求属于环境/构建侧支线排障, 与主线训练/导入任务弱相关.
  - 需要记录构建日志、包依赖和验证结果, 不适合继续污染主线计划正文.
...
## [2026-03-27 14:04:20] [Session ID: a354b352-2b30-435e-b917-cd8fed8e5060] [记录类型]: 分析 FastGS 画质提升参数与 mask 能力

### 目标
- 结合用户当前 `my4_fullcolmap` 的训练结果, 明确哪些训练参数会实质影响清晰度、漂浮棉絮和最终细节.
- 结合源码确认仓库是否已经支持 mask 数据流, 区分“现象”、“可调参数”和“需要改代码的新能力”.
- 如果仓库已有能力, 给出可直接执行的命令.
- 如果仓库没有现成能力, 给出最正确的改造方向, 不把猜测说成结论.

### 两种方向
- 方案A(不惜代价,最佳): 先从当前仓库真实代码路径中确认 mask 与关键训练参数, 再给出一组更稳的最终训练参数建议.
- 方案B(先能用,后面再优雅): 只给经验性参数建议, 暂不确认 mask 能力与数据格式.
...
### 关键问题
1. 当前“有点模糊”更像是分辨率不足、训练步数不足, 还是点云漂浮噪声导致?
2. 这个仓库里哪些参数会直接影响细节保真和漂浮点收敛?
3. 当前数据流是否已经支持前景/天空 mask, 还是只能靠图像预处理后再训练?
4. 如果想专门压掉“空中棉絮”, 最稳的路径是调参、数据清洗, 还是补 mask 机制?

### 状态
**目前在阶段2**
- 2026-03-27 14:04:20 CST: 已完成六文件回读.
- 2026-03-27 14:04:20 CST: 下一步直接检索 `arguments/`、`train.py`、`scene/`、`utils/` 与脚本参数, 先确认真实可调项与 mask 支持现状.

## [2026-03-27 14:12:23] [Session ID: a354b352-2b30-435e-b917-cd8fed8e5060] [记录类型]: 进度更新

### 已完成阶段
- [x] 阶段1: 回读上下文并记录本轮目标
- [x] 阶段2: 检索训练参数与影响路径
- [x] 阶段3: 检索 mask 读入与渲染约束
- [x] 阶段4: 汇总结论并给出下一步建议

### 已落地内容
- 已确认 `run_lyra_colmap_fastgs.sh` 现有脚本本身就暴露了一组关键画质参数, 并非“没有参数可调”.
- 已确认当前默认训练日程对 `30000` 之后的边际收益有限:
  - `densify_until_iter=15000`
  - 强力 prune 只跑到 `<30000`
  - `position_lr_max_steps=30000`
  - `optimizer_step` 在 `>20000` 后降为每 `64` iter 一次
- 已修复 alpha mask 通道判断 bug.
- 已为 COLMAP 数据新增 `mask_dir` / 自动识别 `masks/` 的训练入口.
- 已让 `scripts/run_lyra_colmap_fastgs.sh` 支持 `--mask-dir`.
...
### 状态
**目前已完成**
- 2026-03-27 14:12:23 CST: 本轮“画质参数 + mask 能力”分析与最小改造已完成, 下一步可直接给用户:
  - 更适合 `my4_fullcolmap` 的提质参数建议
  - 新的 mask 使用命令

## [2026-03-27 14:33:53] [Session ID: a354b352-2b30-435e-b917-cd8fed8e5060] [记录类型]: 为 `my4_fullcolmap` 生成室内空中颗粒亮点 mask

### 目标
- 为 `/home/rais/FreeFix/data/my4_fullcolmap/images` 批量生成一套可直接训练使用的 mask.
- 重点针对“室内空中颗粒 / 尘埃 / 小亮点”, 而不是大块天空分割.
- 产出脚本、测试、真实生成结果和调参入口, 方便后续重复执行.

### 两种方向
- 方案A(不惜代价,最佳): 落地一套专门的自动颗粒 mask 生成脚本, 并直接在 `my4_fullcolmap` 上生成首版结果.
- 方案B(先能用,后面再优雅): 只给用户手工制作建议, 暂不写自动脚本.

### 阶段
- [ ] 阶段1: 回读上下文并确认落盘目标
- [ ] 阶段2: 设计颗粒亮点检测算法
- [ ] 阶段3: 实现脚本与测试
- [ ] 阶段4: 在真实数据上生成 mask 并抽样验证

### 关键问题
1. 如何尽量抓住“小而亮的漂浮颗粒”, 同时避免把室内灯具 / 大面积高光误判进去?
2. 输出 mask 的格式是否直接兼容刚补好的 `mask_dir` 训练入口?
3. 是否需要同步产出 debug 预览, 避免后续无法判断误杀范围?

### 状态
**目前在阶段2**
- 2026-03-27 14:33:53 CST: 用户已确认继续做“室内空中颗粒亮点 mask”.
- 2026-03-27 14:33:53 CST: 下一步将实现一套基于亮度、局部残差和小连通域过滤的自动 mask 脚本, 再在真实数据上生成首版结果.

## [2026-03-27 14:43:53] [Session ID: a354b352-2b30-435e-b917-cd8fed8e5060] [记录类型]: 进度更新

### 已完成阶段
- [x] 阶段1: 回读上下文并确认落盘目标
- [x] 阶段2: 设计颗粒亮点检测算法
- [x] 阶段3: 实现脚本与测试
- [x] 阶段4: 在真实数据上生成 mask 并抽样验证
...
- 已在真实数据上生成:
  - 正式训练目录: `/home/rais/FreeFix/data/my4_fullcolmap/masks`
  - 调试预览: `/home/rais/FreeFix/data/my4_fullcolmap/mask_debug_particle_v1`
  - summary: `/home/rais/FreeFix/data/my4_fullcolmap/mask_summary_particle_v1.json`
- 已完成带 mask 的 1 iter 烟雾训练验证:
  - `output/my4_mask_smoke`

### 状态
**目前已完成**
- 2026-03-27 14:43:53 CST: `my4_fullcolmap` 的首版室内颗粒亮点 mask 已生成并通过真实训练链路验证.
- 2026-03-27 14:43:53 CST: 下一步可直接进入:
  - 带 mask 的 30000 正式训练
  - 或带 mask 的 30000 -> 35000 resume 训练

## [2026-03-27 14:46:34] [Session ID: a354b352-2b30-435e-b917-cd8fed8e5060] [记录类型]: 执行带 mask 的 30000 正式训练并守护

### 目标
- 按上一轮确认的参数, 启动 `my4_fullcolmap` 的带 mask 正式训练.
- 训练目标为 `30000` iter, 同时保存 `30000` 的 point cloud / checkpoint / render video.
- 在本轮内持续守护训练进程, 直到确认训练结束或出现需要处理的真实错误.
...
### 状态
**目前在阶段1**
- 2026-03-27 14:46:34 CST: 用户已确认“按建议继续并守护”.
- 2026-03-27 14:46:34 CST: 下一步先检查 `mask_dir`、输出目录与训练命令, 然后直接起正式训练.

## [2026-03-27 14:47:10] [Session ID: a354b352-2b30-435e-b917-cd8fed8e5060] [记录类型]: 进度更新

### 已完成阶段
- [x] 阶段1: 训练前检查关键输入与输出路径

### 检查结果
- `mask_dir=/home/rais/FreeFix/data/my4_fullcolmap/masks` 下已有 `264` 张 mask
- `output/my4_mask_final` 当前不存在, 可以安全按 `--overwrite` 启动
- mask summary 统计:
  - `average_mask_ratio ≈ 0.001128`
  - `max_mask_ratio ≈ 0.002637`

### 状态
**目前在阶段2**
- 2026-03-27 14:47:10 CST: 下一步直接启动 `30000` iter 的带 mask 正式训练, 并持续守护.
...
# 任务计划: 重装 `/workspace/colmap-cuda-install-3.12.6`

## 目标
- 在当前机器上恢复一份可用的 CUDA 版 COLMAP 3.12.6.
- 安装路径固定为 `/workspace/colmap-cuda-install-3.12.6/bin/colmap`, 以兼容仓库现有脚本和文档.
- 保留系统现有 CPU 版 `colmap` 作为回退路径, 不覆盖系统包.

## 两种方向
- 方案A(不惜代价,最佳): 按仓库里已经验证过的源码编译方案完整重建, 并做静态 + 动态验证.
- 方案B(先能用,后面再优雅): 如果源码目录和构建目录还完整, 优先复用已有构建产物快速恢复安装路径, 再补验证.
...
## 关键问题
1. 当前缺的是“安装目录丢失”, 还是源码/构建目录也一起丢失?
2. 当前机器是否仍然满足文档中的稳定组合: CUDA 12.6 + GCC 10 + Ninja + CMake?
3. 是否能直接复用旧构建目录, 还是需要从源码重新配置和编译?
4. 安装后是否真的能跑 GPU SIFT, 而不只是 `-h` 显示 `with CUDA`?

## 状态
**目前在阶段2**
- 2026-03-24 00:00:00 UTC: 已回读项目上下文与构建文档.
- 2026-03-24 00:00:00 UTC: 当前现象是 `/workspace/colmap-cuda-install-3.12.6/bin/colmap` 不存在.
- 2026-03-24 00:00:00 UTC: 下一步先确认旧源码/构建残留和当前编译环境, 再决定是快速恢复还是完整重建.
- 2026-03-24 00:00:00 UTC: 阶段2 已完成环境核查:
  - 旧源码目录、构建目录、安装目录都不存在, 不能走“快速恢复”.
  - 当前系统没有任何 `colmap` 可执行文件.
  - 当前 GPU 已变为 `RTX PRO 6000 Blackwell Server Edition`, `compute_cap=12.0`.
  - `/usr/local/cuda-12.8/bin/nvcc` 实际存在, 但 `PATH` 未带上它.
  - 当前缺少 `gcc-10`、`g++-10`、`ninja-build`, 因此下一步进入完整重建.
...
- [ ] 阶段3: 执行安装或重建
- [ ] 阶段4: 做静态/动态验证并回写记录
- 2026-03-24 00:00:00 UTC: 新的环境约束已确认:
  - 当前用户 `rais` 对 `/workspace` 没有写权限.
  - `sudo -n` 需要密码, 本轮无法直接用 `apt` 或写入 `/workspace`.
  - 因此阶段3 改为无 root 路线:
    - 用 `vcpkg` 在用户目录自举依赖并编译 `colmap[core,cuda]`
    - 安装前缀改为 `~/colmap-cuda-install-3.12.6`
    - 保持目录名与版本号一致, 先完成可用安装, 再视需要处理仓库默认路径兼容

---

# 任务计划: 支持 `data/my4_fullcolmap` 直接跑 COLMAP -> FastGS

## 目标
- 让仓库脚本可以直接使用用户指定的 `my4_fullcolmap` 数据组织.
- 明确要求是不做超分, 只做 COLMAP 预处理与 FastGS / 3DGS 训练.
- 优先改良现有脚本入口, 避免再要求用户手工桥接 `generated_videos -> rgb`.

## 两种方向
- 方案A(不惜代价,最佳): 让现有 COLMAP 包装脚本自动识别 `my4_fullcolmap` 这类真实目录, 直接完成 `prepare/train/render/metrics`.
- 方案B(先能用,后面再优雅): 保持主脚本不动, 单独补一个轻量 wrapper, 专门把这类目录适配到现有 `run_lyra_colmap_fastgs.sh`.

## 阶段
- [x] 阶段1: 回读上下文并续档
- [ ] 阶段2: 定位 `my4_fullcolmap` 真实路径并核对目录结构
- [ ] 阶段3: 设计脚本改造点
- [ ] 阶段4: 实现并验证
- [ ] 阶段5: 回写记录并交付命令

## 关键问题
1. `my4_fullcolmap` 当前真实路径到底在哪里?
2. 它是 `view_id/generated_videos/*.mp4`, 还是已经整理成 `view_id/rgb/*.mp4`, 或者已经带 `images + sparse/0`?
3. 最稳的改法是扩展 `run_lyra_colmap_fastgs.sh`, 还是复用 `run_versecrafter_flashvsr_fastgs.sh` 的部分逻辑抽公共层?
4. 改完后, 用户是否可以直接从这套目录执行 `--phase prepare` 或 `--phase all`?

## 状态
**目前在阶段2**
- 2026-03-27 00:00:00 UTC: 用户明确要求改成支持 `data/my4_fullcolmap`, 不做超分, 只做 3DGS / FastGS.
- 2026-03-27 00:00:00 UTC: 当前已确认仓库内 `data/my4_fullcolmap` 路径不存在, 下一步先定位真实目录, 再按真实结构改脚本.
- 2026-03-27 00:00:00 UTC: [Session ID: 119e250a-72a8-40a1-bad2-1a106f4b536a] 本轮会话使用该 Session ID 记录后续追加内容.

## [2026-03-27 05:48:00 UTC] [Session ID: 119e250a-72a8-40a1-bad2-1a106f4b536a] [进度更新]: `my4_fullcolmap` 脚本支持已完成

- [x] 阶段2: 定位 `my4_fullcolmap` 真实路径并核对目录结构
- [x] 阶段3: 设计脚本改造点
- [x] 阶段4: 实现并验证
- [x] 阶段5: 回写记录并交付命令

### 现象
- 用户要直接使用 `/home/rais/FreeFix/data/my4_fullcolmap`.
- 这套数据不做超分, 只做 FastGS.
- 实际目录已包含 `images/` 与 `sparse/0/`, 说明它已经是可训练 COLMAP 根目录.

### 假设
- 主假设:
  - 改良现有 `scripts/run_lyra_colmap_fastgs.sh`, 让它自动识别“已准备好的 COLMAP 根目录”, 是最正确方案.
- 备选解释:
  - 也可以新增一个专用 wrapper.
  - 但这会继续分裂入口, 性价比更差.
...
- 动态验证2:
  ...
  - 结果: 读取 264 个相机并完成 1 iter smoke train, 最终 `Training complete.`

### 结论
- 当前任务已完成.
- 现有脚本已经支持把 `/home/rais/FreeFix/data/my4_fullcolmap` 当成直接可训练输入.
- `prepare` 会做校验并跳过转换.
- `train` / `all` 可以直接复用这套目录.

## 状态
**目前已完成**
- 2026-03-27 05:48:00 UTC: `scripts/run_lyra_colmap_fastgs.sh` 已支持已准备好的 COLMAP / FastGS 根目录.
- 2026-03-27 05:48:00 UTC: 真实 `my4_fullcolmap` 已完成 `prepare` 校验与 `train` 烟雾验证.

## [2026-03-27 05:52:26 UTC] [Session ID: 119e250a-72a8-40a1-bad2-1a106f4b536a] [后续验证]: 对 `my4_fullcolmap` 执行 1000 iter 短训练

- 目标:
  - 在 1 iter smoke 之后, 再做一轮更接近真实训练节奏的短训练.
  - 验证这套数据在连续训练过程中不会中途报错, 并为是否建议直接上 `--phase all -r 1` 提供动态证据.
- 计划执行:
...
## [2026-03-27 05:54:35 UTC] [Session ID: 119e250a-72a8-40a1-bad2-1a106f4b536a] [验证结果]: `my4_fullcolmap` 短训练与 render 已完成

- 现象:
  - `1000 iter` 短训练已完整跑完.
  - 随后 `render --iteration 1000` 也已完整跑完 train/test 两套视角.
- 动态证据:
  - 训练:
...
## 状态
**目前已完成**
- 2026-03-27 05:54:35 UTC: `my4_fullcolmap` 已完成 1000 iter 短训练与 render 验证.
- 2026-03-27 05:54:35 UTC: 当前下一步建议是正式执行完整 `--phase all -r 1`.

## [2026-03-27 05:54:35 UTC] [Session ID: 119e250a-72a8-40a1-bad2-1a106f4b536a] [新任务]: `my4_fullcolmap` 从 30000 续训到 50000, 并在 40000 / 50000 导出视频

- 目标:
  - 基于已有 `30000` 步结果继续训练到 `50000`.
  - 在 `40000` 和 `50000` 两个阶段都产出可直接查看的视频.
- 当前主假设:
  - 需要先确认当前仓库是否真的保留了“可续训”的 checkpoint, 而不是只有 `point_cloud.ply`.
- 最强备选解释:
  - 如果没有 checkpoint, 可能只能从 `point_cloud` 恢复渲染评估, 但不能无损继续优化.
  - 这时就需要改训练脚本, 或者重新从头按更长步数跑正式训练.
- 下一步:
  - 先核查现有 `output` 目录里是否存在 `30000` 步 checkpoint / cfg / point_cloud.
  - 再核对 `train.py` 对 `--start_checkpoint` / `--checkpoint_iterations` 的真实支持情况.

## [2026-03-27 06:56:51 UTC] [Session ID: 019d2d07-3c10-70b0-a340-22753598e9ff] [记录类型]: `my4_mask_final` 正式训练守护中发现 CUDA 非法显存访问

### 目标
- 继续推进 `my4_fullcolmap + mask` 的正式训练, 但先把这次中途崩溃查清楚.
- 明确区分:
  - 现象: 训练在哪一步炸, 输出目录留下了什么
  - 假设: 是高斯数量 / 调度过激, 还是某个坏状态触发 rasterizer
  - 结论: 必须等最小复现和验证后再下
...
- [ ] 阶段3: 缩短复现路径并定位触发条件
- [ ] 阶段4: 选择修复或稳态参数并重新启动
- [ ] 阶段5: 完成正式训练, 核查视频 / metrics / checkpoint
...
- 守护时拿到的动态日志显示:
  - 训练曾正常推进到约 `14240/30000`
  - 随后在 `diff_gaussian_rasterization_fastgs` 的 forward 里报:
    - `torch.AcceleratorError: CUDA error: an illegal memory access was encountered`
- 当前输出目录只留下:
  - `output/my4_mask_final/cameras.json`
  - `output/my4_mask_final/cfg_args`
  - `output/my4_mask_final/input.ply`
- 还没有生成:
  - `checkpoints/`
...
### 当前假设
- 主假设:
  - 这次 crash 更像是训练状态在 densify 期间变得过激, 最终把 rasterizer 顶进非法访问.
- 最强备选解释:
  - 不是全局高斯数量问题, 而是某一轮渲染输入出现坏值, 例如 NaN / 异常尺度 / 异常相机组合.
- 推翻主假设需要的证据:
  - 如果在保留同一训练状态但降低 densify 激进度后仍在相近位置炸, 那更像是坏状态或 kernel 兼容性问题.

### 下一步
- 先核对训练与 densify 代码路径, 确认最合适的最小复现命令.
- 争取把下一次复现改成:
  - 带 `CUDA_LAUNCH_BLOCKING=1`
  - 带更密的 checkpoint / save iteration
  - 尽量在接近崩点前保留证据

## [2026-03-27 07:20:00 UTC] [Session ID: 019d2d07-3c10-70b0-a340-22753598e9ff] [进度更新]: `illegal memory access` 已缩小到 `11240` 左右
...
### 新的动态证据
- 在 `CUDA_LAUNCH_BLOCKING=1` 下, 同参数复现不再模糊:
  - 崩溃发生在约 `11240/15000`
  - 报错位置落在 `gaussian_renderer/__init__.py` 的:
    - `visibility_filter : (radii > 0).nonzero()`
  - 错误类型变为更具体的:
    - `cudaErrorInvalidAddressSpace`
- 当前没有中间 checkpoint, 因为这轮设置的 `12000/13000/14000` 都还没来得及保存.

### 当前判断
- 现象:
  - 真正出问题的不是脚本收尾, 也不是 render 阶段.
  - 是训练主循环里某次 `render_fastgs` 返回了坏的 `radii` 状态.
- 假设:
  - 崩点紧邻 `11200` 这个 densify 间隔点.
  - 当前最值得先验证的是“11200 那次 densify 之后引入了坏状态”.
- 最小后续动作:
  - 重跑一轮, 把 checkpoint 提前到 `10000/10500/11000`.
  - 若 `11000` checkpoint 可得, 先从它 resume.
  - 续训时把 `densify_until_iter` 收到 `11000`, 先验证稳定性.

## [2026-03-27 07:28:00 UTC] [Session ID: 019d2d07-3c10-70b0-a340-22753598e9ff] [进度更新]: 激进提质参数被判定为当前主嫌疑

### 新增静态证据
- `train_base.sh` 对大多数场景使用:
  - `--densification_interval 500`
  - `--grad_abs_thresh` 通常不低于 `0.0006 ~ 0.0015`
- 本轮崩溃参数使用:
  - `--densification_interval 100`
  - `--loss_thresh 0.08`
  - `--grad_abs_thresh 0.0010`
  - `--mult 0.7`
- 结合 `docs/fastgs-train-scripts.md` 的现有说明:
  - `loss_thresh` 越小, 高误差像素越多
  - `grad_abs_thresh` 越小, split 越积极
  - `mult` 越大, compact box 越保守, 可能覆盖更多 tiles

### 当前判断
- 主假设:
  - 不是单一参数独立出错.
  - 而是“更频繁 densify + 更积极 split + 更保守 compact box”叠加, 让当前室内全分辨率训练不稳定.
- 下一步:
  - 改用更接近 base 的稳态参数做一轮到 `16000` 的稳定性验证.
  - 若通过, 直接以该组参数继续推进正式训练.

## [2026-03-27 07:40:00 UTC] [Session ID: 019d2d07-3c10-70b0-a340-22753598e9ff] [结论更新]: mask 训练链路的真实问题是“单边涂黑 GT”, 不是“真正忽略 mask 像素”
...
- 旧实现里:
  ...
  - 但 render 图像没有同步乘同一份 mask
- 这会让被 mask 的像素从“忽略区域”变成“必须拟合成黑色”的监督目标.
- `utils/fast_utils.py` 的 photometric loss / metric_map 也继承了同样问题.

### 当前结论
- 上一阶段“主要是参数过激”的口径需要收回一半:
  - 参数激进确实会放大问题.
  - 但真正把 mask 训练拖进不稳定状态的, 是 loss 语义不对.
- 当前已完成修复:
  - 训练主 loss
  - FastGS densify 用的 photometric loss
  - metric_map 生成路径
  - Camera 对 alpha mask 的持久化

### 下一步
- 直接启动正式 `30000` 步 mask 训练.
- 采用当前已验证可稳过 `9000` 的参数:
  - `densification_interval=500`
  - `densify_until_iter=15000`
...
## [2026-03-27 15:34:00 UTC] [Session ID: 019d2d07-3c10-70b0-a340-22753598e9ff] [进度更新]: 外部 CoherentGS 任务已结束, 可以重新做正式守护

### 新的动态证据
- `nvidia-smi --query-compute-apps=pid,process_name,used_gpu_memory --format=csv,noheader`
  - 当前返回为空
- `ps -ef | rg 'simple_deblur_difix.py|train.py|render.py|metrics.py|pixi run python train.py|python train.py'`
  - 当前没有残留训练进程

### 当前判断
- 之前“共卡干扰”这一层不确定性已经消失.
- 可以重新启动正式 `30000` 步训练, 并继续守护.

### 下一步
- 直接启动:
  - `--phase all`
  - `--video-iterations 10000,20000,30000`
  - `--model-path output/my4_mask_final_v3`
- 目标:
  - 训练
  - 阶段 checkpoint
  - 阶段 point cloud
...
## [2026-03-27 15:36:30 UTC] [Session ID: 019d2d07-3c10-70b0-a340-22753598e9ff] [策略调整]: 从“mask 从 0 直接训”切到“两段式 structure-first”

### 新的动态证据
- 外部共卡任务已经消失后:
  - `output/my4_mask_final_v3`
  - 仍然在约 `5220` 附近 crash
- 说明问题不只是共卡干扰.
- 当前更可靠的现象是:
  - true no-mask `9000` 稳定
  - 带 mask 从 `0` 开始训仍然不稳
  - 崩点持续贴近 densify 时段

### 当前判断
- 主假设:
  - mask 从早期就参与 densify / metric_map, 仍会把增点策略带偏, 或放大 rasterizer 的不稳定区域.
- 更稳的落地策略:
  - 第一段:
    - 不带 mask
    - 先把结构与主干几何跑稳, 并拿 checkpoint
  - 第二段:
    - 从 checkpoint 恢复
    - 带 mask
    - 把 `densify_until_iter` 卡在当前步数, 不再增点
    - 只做后段清洁与收敛

### 下一步
- 先做最小验证:
  - no-mask 跑到 `10000` 并保存 checkpoint
  - 从 `10000` checkpoint 带 mask 续到 `11000`
  - 同时关闭后续 densify
- 若该 smoke 通过:
  - 按同策略继续:
    - 10000 -> 20000
...
## [2026-03-27 15:39:30 UTC] [Session ID: 019d2d07-3c10-70b0-a340-22753598e9ff] [策略调整]: 改用每 `1000` 步落 checkpoint 的 guarded 训练

### 现象
- 即使外部共卡任务已结束:
  - mask from scratch 仍可能在 `5k` 左右 crash
- 即使切到 no-mask:
  - 长于 `9000` 的连续运行也可能在接近 `10000` 时 crash

### 当前判断
- 这份 FastGS + 当前 GPU / CUDA 组合存在明显的长跑不稳定性.
- 但短段训练可以多次成功.
- 当前最稳妥的交付策略不是继续赌“一口气 30000”.
- 而是:
  - 每 `1000` 步保存一次 checkpoint / point cloud
  - 用 wrapper 的 `--start-checkpoint` 逐段前推
  - 一旦某段失败, 只回退最近一个稳定锚点

### 下一步
- 启动 `1000` 步分段 guarded 训练.
- 先以带 mask 版本推进.
- 若某一段重复失败, 再切到:
  - 无 mask 过 densify
  - mask 负责后段清洁
## [2026-03-27 07:47:35 UTC] [Session ID: 6e9061e6-0ba0-4e70-811a-eeaa92ee05ed] [进度更新]: 接管 `my4_mask_guarded_v4` 守护会话并确认最新稳定锚点

### 当前阶段
- [x] 阶段1: 接管并回读六文件上下文
- [x] 阶段2: 守护正式训练并观察运行态
- [x] 阶段3: 确认最近成功 checkpoint 与当前失败区间
- [ ] 阶段4: 用新的 seed 重启失败分段, 并改成自动重试守护
- [ ] 阶段5: 推进到 `30000`, 然后补 render / video / metrics
...
- 当前这条链路已经成功推进并落盘:
  ...
  - `ckpt_10000.pth`
  - `ckpt_11000.pth`
- 同步确认 `point_cloud/iteration_9000`、`10000`、`11000` 也已经生成.
- GPU 当前无其他训练进程占用:
  - `nvidia-smi --query-compute-apps=pid,process_name,used_gpu_memory --format=csv,noheader` 返回空.
- 最新失败段不是 `9000 -> 10000` 或 `10000 -> 11000`.
- 真正失败的是:
  - `11000 -> 12000`
  - 在大约 `11510` 左右报 `torch.AcceleratorError: CUDA error: an illegal memory access was encountered`

### 当前判断
- 现象:
  - 分段 + 换 seed 的 guarded 策略是有效的, 因为已经把稳定锚点从 `6000` 推进到了 `11000`.
- 假设:
  - 当前问题仍是这套 FastGS + 现机 GPU / CUDA 扩展的长跑随机不稳定.
  - 不是 checkpoint 机制失效, 也不是 mask 入口失效.
- 最小下一步:
  - 从 `ckpt_11000.pth` 重新起 `12000` 这一段.
  - 换一个新的 seed.
  - 用自动重试守护把后续 `13000 ... 30000` 一起包住, 避免再手工盯每一段.
## [2026-03-27 08:02:10 UTC] [Session ID: 6e9061e6-0ba0-4e70-811a-eeaa92ee05ed] [进度更新]: 已跨过 densify 边界并稳定推进到 `19000`

### 当前阶段
- [x] 阶段1: 接管并回读六文件上下文
- [x] 阶段2: 守护正式训练并观察运行态
- [x] 阶段3: 确认最近成功 checkpoint 与当前失败区间
- [x] 阶段4: 用新的 seed 重启失败分段, 并改成自动重试守护
- [ ] 阶段5: 推进到 `30000`, 然后补 render / video / metrics
...
- 这一轮自动守护已经连续拿到:
  ...
  - `ckpt_18000.pth`
  - `ckpt_19000.pth`
- `14000` 这段曾在 attempt 1 很早就报 `illegal memory access`, 但自动换 seed 后 attempt 2 成功通过.
- `15000` 已经落稳, 说明当前训练已经跨过 `densify_until_iter=15000` 的边界.
- `16000` 之后的推进明显更顺, 当前已启动 `20000` 这一段.
- 同期又观测到外部 GPU 占用重新出现:
  - `simple_deblur_difix.py`(CoherentGS)
  - 但即使在这个干扰下, `16000 ~ 19000` 仍连续成功.

### 当前判断
- 现象:
  - 当前 guarded 策略已经不只是“能救火”, 而是确实在把正式训练持续往前推.
- 结论:
  - `15000` 之后稳定性显著改善.
  - 继续守到 `30000`, 再做视频与指标导出, 是当前最值得坚持的路径.
