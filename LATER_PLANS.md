...
## [2026-03-27 08:21:30 UTC] [Session ID: 019d2d07-3c10-70b0-a340-22753598e9ff] 暂停 `my4_mask_guarded_v4` 继续训练, 优先回查 COLMAP 位姿与素材质量
- 已观察到的事实:
  - `output/my4_mask_guarded_v4` 已成功推进到 `30000`, 并已产出 `10000 / 20000 / 30000` 三档视频与 `results.json`.
  - 指标仍在上涨:
    - `ours_10000`: `PSNR 25.1422`, `SSIM 0.8714`, `LPIPS 0.2271`
    - `ours_20000`: `PSNR 25.9369`, `SSIM 0.8829`, `LPIPS 0.2089`
    - `ours_30000`: `PSNR 26.0466`, `SSIM 0.8845`, `LPIPS 0.2060`
  - 但用户主观观察到当前渲染仍有“重影”.
- 当前假设:
  - 重影更像是前处理阶段的相机位姿 / 素材问题, 不一定是训练步数不足.
  - 这只是候选假设, 还没有做新的动态验证.
- 下次继续时的优先顺序:
  - 先别继续冲 `35000` 或 `50000`.
  - 先核对 `my4_fullcolmap` 的相机轨迹、稀疏点云和 worst-view.
  - 如果静态与动态证据都指向位姿偏差, 再决定是重跑 COLMAP, 还是回到素材侧做更干净的抽帧 / 筛帧 / 去动态.
- 额外提醒:
  - `task_plan.md`、`WORKLOG.md`、`notes.md` 当前都已经超过 `1000` 行.
  - 下次正式续档前, 应先做一次续档和 `continuous-learning`, 再继续往主线日志里写.

## [2026-03-27 21:59:40] [Session ID: 245310] `my5` 首轮 30000 训练已完成后的后续动作
- 当前 `my5` 已完成:
  - `1/3` 图量 `COLMAP prepare`
  - 无训练 mask 的 `30000` 轮 FastGS 训练
- 如果下一步要继续判断效果与调优方向, 建议按这个顺序:
  - 先跑 `render.py` / `metrics.py`, 固化 `my5_nomask_v1` 的客观结果
  - 再对比关键视角视频, 判断问题更像位姿、素材还是训练超参数
  - 最后再决定是否需要重新引入深度辅助信息, 但不要直接把 `merged_mask.mp4` 接回普通训练 mask 语义

## [2026-03-28 02:24:00 UTC] [Session ID: 79642ac4-ccdf-404a-967b-1342f85cc2bd] `my5` 下一步优先做局部 COLMAP 对照, 不要先继续堆训练步数
- 当前新增证据已经表明:
  - `view 7` 不只平移轨迹最差
  - 朝向连续性也全机位最差
  - 相邻 pair `6-7` / `7-8` 的相对位姿变化也最不稳
- 如果后续继续 `my5`, 更推荐优先做:
  - 只围绕 `view 6 / 7 / 8`
  - 尤其是 `frame 1 ~ 8`
  - 做一次局部 COLMAP / matcher 对照
- 在这一步之前, 不建议把主要精力继续放在:
  - 拉更多训练迭代
  - 或者直接做新一轮训练超参微调

## [2026-03-28 02:49:00 UTC] [Session ID: 7f1d2edd-4a39-4aee-b0fd-79f9701c57e7] `my5` 的下一步更适合做筛帧对照, 不适合继续缩小局部 COLMAP 窗口
- 这轮局部 COLMAP 对照已经完成:
  - `local_fix`
  - `local_refine`
- 两版都没有把 `view 7` 前段明显修顺.
- 因此如果后续继续, 更推荐优先做:
  - 去掉 `view 7 frame 3 ~ 6` 的筛帧对照
  - 或者放大到 `view 5 / 6 / 7 / 8 + frame 1 ~ 12`
- 当前不推荐优先做:
  - 继续缩成更小的局部 COLMAP 子集
  - 继续只靠训练步数或小超参调优试图掩盖这个问题

## [2026-03-27 18:55:48 UTC] [Session ID: ebb3c562-1702-4ea2-8e98-ed18d0a9bada] `my5` 筛帧对照前先补 frame-gap 口径保护
- 这轮 `widecontext` 也已经验证:
  - 放大到 `view 5 / 6 / 7 / 8 + frame 1 ~ 12` 仍没有把 `view 7` 修顺
- 因此下一步如果继续做删帧 / 筛帧实验, 更稳的顺序是:
  - 先扩展 `scripts/analyze_colmap_pose_continuity.py`
  - 显式区分“真正相邻帧(frame gap == 1)”和“跨 gap 跳跃”
- 否则一旦删掉 `view 7 frame 3 ~ 6`, 当前连续性统计会把 `2 -> 7` 之类的跳跃误当成邻帧, 污染对照结论
- 如果还想继续做 COLMAP 对照, 优先尝试:
  - 更保守的 mapper / BA 参数
  - 而不是继续改窗口大小
